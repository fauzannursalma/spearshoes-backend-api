                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <table class="table-auto w-full">
                        <tbody>
                            ...
                            <tr>
                                <th class="border px-6 py-4 text-right">Transaction Date</th>
                            ...
                            <tr>
                                ...
                                <td class="border px-6 py-4">
                                    @if ($transaction->status == 'PENDING')
                                        <span
                                            class="bg-gray-400 text-white px-2 py-1 rounded-md">{{ $transaction->status }}</span>
                                    @elseif($transaction->status == 'SHIPPING')
                                        <span
                                            class="bg-blue-500 text-white px-2 py-1 rounded-md">{{ $transaction->status }}</span>
                                    @elseif($transaction->status == 'SHIPPED')
                                        <span
                                            class="bg-blue-500 text-white px-2 py-1 rounded-md">{{ $transaction->status }}</span>
                                    @elseif($transaction->status == 'SUCCESS')
                                        <span
                                            class="bg-green-500 text-white px-2 py-1 rounded-md">{{ $transaction->status }}</span>
                                    @elseif($transaction->status == 'FAILED')
                                        <span
                                            class="bg-red-500 text-white px-2 py-1 rounded-md">{{ $transaction->status }}</span>
